...
<span className="text-xs font-medium bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 px-2 py-0.5 rounded">UPDATED in v0.9.6</span>

The `db` command group manages the entity embedding database used for organization, person, role, and location qualification.

**Global `--db-version` flag** <span className="text-xs font-medium bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 px-2 py-0.5 rounded">v0.9.6</span>: Controls which database version filenames are used. Defaults to the latest (v3). Use `corp-extractor --db-version=2 db download` to fetch v2 files. On download, a backwards-compatibility symlink `entities-v2.db` → `entities-v3.db` is created automatically.

```bash
# Show database status
corp-extractor db status

# Search for an organization (USearch HNSW)
...
corp-extractor db search-people "Tim Cook" --hybrid  # Hybrid text + embeddings search
...
# Download/upload from HuggingFace Hub (includes USearch indexes)
corp-extractor db download                        # Lite version + USearch indexes (default)
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Use existing dump file (.bz2, .zst, or .gz)</td>
    </tr>
    <tr>
...
- 3-thread parallel pipeline (reader → embedder → writer) for fast import
- Multi-record person import (one per position+org, max 10 per person)
...
- Reverse org→person mappings with date qualifiers (P169 CEO, P488 chairperson)
- Auto-canonicalization at end of import with recency tiebreaker
- Imports locations with hierarchical parent relationships (v0.9.4)
- Supports `.bz2`, `.zst`/`.zstd` compressed dumps
...
<table>
  ...
  <tbody>
    <tr>
      <td><code>--db-version N</code></td>
      <td>Database schema version for filenames (global flag, e.g. <code>corp-extractor --db-version=2 db download</code>)</td>
      <td>latest (3)</td>
    </tr>
    <tr>
      <td><code>--db PATH</code></td>
      <td>Database file path</td>
      <td>~/.cache/corp-extractor/entities-v3.db</td>
    </tr>
    <tr>
...
See [ENTITY_DATABASE.md](https://github.com/corp-o-rate/statement-extractor/blob/main/ENTITY_DATABASE.md) for complete build and publish instructions.
