...
```bash
# Download lite version + USearch indexes (default)
corp-extractor db download
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Core fields only (no embedding tables)</td>
    </tr>
    <tr>
      <td><code>entities-v3.db</code></td>
      <td>Full records with all embedding tables</td>
    </tr>
    <tr>
      <td><code>*_usearch.bin</code></td>
      <td>USearch HNSW indexes for search</td>
    </tr>
  </tbody>
</table>

**Storage location:** `~/.cache/corp-extractor/entities-v3.db`
...
The database uses SQLite with [sqlite-vec](https://github.com/asg017/sqlite-vec) for embedding storage and [USearch](https://github.com/unum-cloud/usearch) HNSW indexes for sub-millisecond approximate nearest neighbor search.
...
-- Embedding tables (full database only — dropped in lite v3)
...
-- Metadata table (v0.9.6)
CREATE TABLE db_info (key TEXT PRIMARY KEY, value TEXT NOT NULL);
...
# Search by name (uses USearch HNSW index for fast ANN search)
...
**Fast import extras** (optional, ~3x JSON parsing + 6x decompression):
```bash
pip install "corp-extractor[fast-import]"  # Adds orjson + indexed_bzip2
```

The dump import uses a 3-thread parallel pipeline (reader → embedder → writer) for overlapping I/O, CPU encoding, and disk writes. Each person now yields multiple records (one per position+org combination), and canonicalization runs automatically at the end.
...
# 2. Post-import: generate embeddings, build USearch indexes, VACUUM
corp-extractor db post-import

# 3. Link equivalent records
...
**Database not found:**
```
Error: Database not found at ~/.cache/corp-extractor/entities-v3.db
```
