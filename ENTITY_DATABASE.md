...
#### Embedding Tables (sqlite-vec) — Full Database Only

The full database contains embedding tables for rebuilding USearch indexes. The lite database drops all embedding tables to reduce size — search uses USearch indexes directly.

| Table | Columns |
|-------|---------|
| `organization_embeddings` | org_id INTEGER, embedding FLOAT[768], distance_metric=cosine |
| `organization_embeddings_scalar` | org_id INTEGER, embedding INT8[768], distance_metric=cosine |
| `person_embeddings` | person_id INTEGER, embedding FLOAT[768], distance_metric=cosine |
| `person_embeddings_scalar` | person_id INTEGER, embedding INT8[768], distance_metric=cosine |
...
Contains `schema_version = 3` for version detection.

#### USearch HNSW Indexes

Pre-built HNSW indexes enable sub-millisecond approximate nearest neighbor search:

| File | Description |
|------|-------------|
| `organizations_usearch.bin` | HNSW index for organization embeddings |
| `people_usearch.bin` | HNSW index for person embeddings |

Indexes are stored alongside the database file and built via `corp-extractor db post-import` or `corp-extractor db build-index`. They are included in `db upload` and `db download`.
...
| Config | Contents |
|--------|----------|
| `entities-v3-lite.db` | Core fields only (no record data, no embedding tables) |
| `entities-v3.db` | Full records with source metadata + all embedding tables |
...
The lite version + USearch indexes is recommended for most use cases. The full database is only needed to rebuild USearch indexes from scratch.
...
The global `--db-version` flag controls which database version filenames are used for download/upload operations. When omitted, defaults to the latest version (v3).

**Storage location:** `~/.cache/corp-extractor/entities-v3.db` (or `entities-v3-lite.db` for lite). On download, a backwards-compatibility symlink `entities-v2.db` → `entities-v3.db` is created automatically.
...
Using USearch HNSW indexes (v0.9.5+):
...
# Create lite version for deployment (drops record data + all embedding tables)
corp-extractor db create-lite ~/.cache/corp-extractor/entities-v3.db
```
...
# Run post-import after completing import
corp-extractor db post-import
```
